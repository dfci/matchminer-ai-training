#!/usr/bin/env python3
"""
Patient-centric trial retrieval using Qwen3 embedding baseline.

For each patient, finds the top N matching trial spaces (only open at time of treatment).
...
def parse_args():
    parser = argparse.ArgumentParser(description="Patient-centric trial retrieval (Qwen3 baseline) for SOC")
...
def main():
    ...
    if args.output_file is None:
        args.output_file = str(baseline_dir / "patient_centric_candidates.csv")
    if args.shard_dir is None:
        args.shard_dir = str(baseline_dir / "shards_patient_centric")
    ...
    for shard_idx in range(0, total_patients, args.shard_size):
        ...
        for i in range(shard_idx, end_idx):
            ...
            # Filter spaces open at patient's treatment time
