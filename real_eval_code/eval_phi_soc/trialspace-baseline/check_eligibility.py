#!/usr/bin/env python3
"""
LLM-based eligibility checking for patient-trial matches using baseline LLM.

This script uses openai/gpt-oss-120b via vLLM to determine if trial spaces are
reasonable considerations for patients (or vice versa).
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM-based eligibility checking (baseline) for SOC")
...
def main():
    ...
    if args.input is None:
        if args.mode == "patient_centric":
            args.input = str(baseline_dir / "patient_centric_candidates.csv")
        else:
            args.input = str(baseline_dir / "trial_centric_candidates.csv")

    if args.output_dir is None:
        if args.mode == "patient_centric":
            args.output_dir = str(baseline_dir / "patient_centric_eligibility_checks")
        else:
            args.output_dir = str(baseline_dir / "trial_centric_eligibility_checks")
    ...
    for i in range(start_idx, candidates.shape[0]):
        ...
        if (num_in_batch == args.batch_size) or (i == (candidates.shape[0] - 1)):
            ...
            if args.mode == "patient_centric":
                output_file = output_dir / f"patient_centric_eligibility_through_{i}.csv"
            else:
                output_file = output_dir / f"trial_centric_eligibility_through_{i}.csv"
